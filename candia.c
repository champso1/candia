...
int main(int argc,char *argv[])
{
	...
	double ****S;
	double A[37][ITERATIONS][GRID_PTS]={0.};
	double *B[37][ITERATIONS][ITERATIONS];
	double *C[37][ITERATIONS][ITERATIONS][ITERATIONS];
	...
	if (order==1)
		for (i=0;i<=36;i++)
			for (j=0;j<=ITERATIONS-1;j++)
				for (k=0;k<=ITERATIONS-1;k++)
					B[i][j][k]=Calloc(GRID_PTS,sizeof(double));
			...
			for (j=0;j<=ITERATIONS-1;j++)
				for (k=0;k<=ITERATIONS-1;k++)
					for (l=0;l<=ITERATIONS-1;l++)
						C[i][j][k][l]=Calloc(GRID_PTS,sizeof(double));
	...
	for (nf=nfi;;nf++)
	{
		...
		if (alpha0!=alpha1)
		{
			...
			for (;Qtab[qct]<=Q[nf+1] && qct<sizeof(Qtab)/sizeof(double);qct++)
			{
				...
				for (int ii = 0; ii < ((sizeof xtab) / (sizeof *xtab)); ++ii)
					fprintf(fp, "%.1e ", xtab[ii]);
				fprintf(fp, "\n");
				for (int ii = 0; ii < ((sizeof xtab) / (sizeof *xtab)); ++ii)
					fprintf(fp, "%d ", ntab[ii]);
				fprintf(fp, "\n");
		...
		if (HFT && vfns && order==2 && Q[nf+2]!=0.)
		{
			...
			for (k=0;k<=GRID_PTS-2;k++)
			{
				for (i=1;i<=nf;i++)
					for (j=i;j<=i+6;j+=6)
						C[j][0][0][0][k]+=pow(aux/(4.*Pi),2.)*convolution(k,A2ns,C[j][1][0][0]);

				S[0][0][0][k]+=pow(aux/(4.*Pi),2.)*(convolution(k,A2gq,S[0][1][1])+convolution(k,A2gg,S[0][0][1]));

				C[nf+1][0][0][0][k]=C[nf+7][0][0][0][k]=0.5*pow(aux/(4.*Pi),2.)*(convolution(k,A2hq,S[0][1][1])+convolution(k,A2hg,S[0][0][1]));
			}
		}
	}
}
