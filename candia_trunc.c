...
extern void hplog_(double *, int *, dcomplex *, dcomplex *, dcomplex *,
                   dcomplex *, double *, double *, double *, double *, double *,
                   double *, double *, double *, int *, int *);
extern void struc_(double*, double*, int*, double*, double*, double*, double*, double*, double*, double*, double*);
extern void a02m_(double *, double *, double *, double *, int *, int *, int *,
                  int *);
...
int main(int argc,char *argv[])
{
	...
	{
		printf("\nUSAGE\n\n");
		printf("./candia_trunc.x <perturbative_order> <truncation_index> <input_model> <kr> <fns> <ext>\n\n");
	...
	//Allocation
	S=Malloc((trunc+1)*sizeof(double***));

	for (i=0;i<=trunc;i++)
	{
		S[i]=Malloc(37*sizeof(double**));

		for (j=0;j<=36;j++)
		{
			S[i][j]=Malloc(ITERATIONS*sizeof(double*));

			for (k=0;k<=ITERATIONS-1;k++)
				S[i][j][k]=Calloc(GRID_PTS,sizeof(double));
		}
	}
	...
	if (input<0 && input>4)
	{
		printf("\nInvalid argument for <input_model>\n");
		return 0;
	}
	...
	fprintf(stderr, "ntab: ");
	for (int ii=0; ii<((sizeof xtab)/(sizeof *xtab)); ++ii)
		fprintf(stderr, "%d ", ntab[ii]);
	fprintf(stderr, "\n");
	fprintf(stderr, "ntab: ");
	for (int ii=0; ii<((sizeof xtab)/(sizeof *xtab)); ++ii)
		fprintf(stderr, "%.2e ", X[ntab[ii]]);
	fprintf(stderr, "\n");
	...
	if (strlen(argv[6])>3)
	{
		printf("\nInvalid argument for <ext>\n");
		return 0;
	}

	sprintf(extension,argv[6]);
	...
	for (i=0;i<=GRID_PTS-2;i++)
	{
		...
		switch (input)
		{
			case 0: //Les Houches toy model
				...
				S[0][0][0][i]=xg_3(aux);
				S[0][31][0][i]=xuv_3(aux)+2.*xub_3(aux)+xdv_3(aux)+2.*xdb_3(aux)+2.*xs_3(aux);

				S[0][7][0][i]=xub_3(aux);
				S[0][1][0][i]=xuv_3(aux)+S[0][7][0][i];
				S[0][8][0][i]=xdb_3(aux);
				S[0][2][0][i]=xdv_3(aux)+S[0][8][0][i];
				S[0][3][0][i]=S[0][9][0][i]=xs_3(aux);

				break;

			case 1: //MRST parametrization

				S[0][7][0][i]=0.2*xS_1(order,aux)-0.5*xD_1(order,aux);
				S[0][1][0][i]=xuv_1(order,aux)+S[0][7][0][i];
				S[0][8][0][i]=0.2*xS_1(order,aux)+0.5*xD_1(order,aux);
				S[0][2][0][i]=xdv_1(order,aux)+S[0][8][0][i];
				S[0][3][0][i]=S[0][9][0][i]=0.1*xS_1(order,aux);

				S[0][0][0][i]=xg_1(order,aux);
				S[0][31][0][i]=S[0][1][0][i]+S[0][7][0][i]+S[0][2][0][i]+S[0][8][0][i]+2.*S[0][3][0][i];
			...
			case 2: //MRST grid
				...
				S[0][0][0][i]=xg;
				S[0][31][0][i]=xuv+xdv+2.*(xub+xdb+xs);

				S[0][7][0][i]=xub;
				S[0][1][0][i]=xuv+xub;
				S[0][8][0][i]=xdb;
				S[0][2][0][i]=xdv+xdb;
				S[0][3][0][i]=S[0][9][0][i]=xs;
			...
			case 3: //Alekhin parametrization
				...
				S[0][7][0][i]=xus_2(order,aux);
				S[0][1][0][i]=xuv_2(order,aux)+S[0][7][0][i];
				S[0][8][0][i]=xds_2(order,aux);
				S[0][2][0][i]=xdv_2(order,aux)+S[0][8][0][i];
				S[0][3][0][i]=S[0][9][0][i]=xss_2(order,aux);
			...
			case 4: //Alekhin grid
				...
				S[0][0][0][i]=pdfs[3];
				S[0][31][0][i]=pdfs[1]+pdfs[2]+2.*(pdfs[4]+pdfs[6]+pdfs[5]);

				S[0][7][0][i]=pdfs[4];
				S[0][1][0][i]=pdfs[1]+pdfs[4];
				S[0][8][0][i]=pdfs[6];
				S[0][2][0][i]=pdfs[2]+pdfs[6];
				S[0][3][0][i]=S[0][9][0][i]=pdfs[5];
	...
	for (nf=nfi;;nf++)
	{
		...
		{
			for (j=13;j<=18;j++) S[0][j][0][i]=S[0][j-12][0][i]-S[0][j-6][0][i];

			S[0][25][0][i]=0.;
			for (j=13;j<=18;j++) S[0][25][0][i]+=S[0][j][0][i];

			for (j=26;j<=30;j++) S[0][j][0][i]=S[0][13][0][i]-S[0][j-12][0][i];

			for (j=19;j<=24;j++) S[0][j][0][i]=S[0][j-18][0][i]+S[0][j-12][0][i];

			S[0][31][0][i]=0.;
			for (j=19;j<=24;j++) S[0][31][0][i]+=S[0][j][0][i];

			for (j=32;j<=36;j++) S[0][j][0][i]=S[0][19][0][i]-S[0][j-12][0][i];
		}
		...
		if (alpha0!=alpha1)
		{
			...
			for (i=0;i<=ITERATIONS-2;i++)
			{
				...
				//Evolution of 0-coefficients
				for (k=0;k<=GRID_PTS-2;k++)
				{
					//Singlet
					S[0][31][i+1][k]=RecRel_A(S[0][31][i],k,P0qq)+RecRel_A(S[0][0][i],k,P0qg);

					S[0][0][i+1][k]=RecRel_A(S[0][31][i],k,P0gq)+RecRel_A(S[0][0][i],k,P0gg);

					//Nonsinglet
					for (j=32;j<=30+nf;j++)
						S[0][j][i+1][k]=RecRel_A(S[0][j][i],k,P0NS);
				...
				if (order>0)
				{
					//Offset of 1-coefficients
					for (k=0;k<=GRID_PTS-2;k++)
						for (j=0;j<=36;j++)
							S[1][j][i+1][k]=-S[0][j][i+1][k]*beta1/(4.*Pi*beta0)-S[1][j][i][k];

					//Evolution of 1-coefficients
					for (k=0;k<=GRID_PTS-2;k++)
					{
						//Singlet
						S[1][31][i+1][k]+=RecRel_B(S[0][31][i],S[1][31][i],k,P0qq,P1qq)+RecRel_B(S[0][0][i],S[1][0][i],k,P0qg,P1qg);

						S[1][0][i+1][k]+=RecRel_B(S[0][31][i],S[1][31][i],k,P0gq,P1gq)+RecRel_B(S[0][0][i],S[1][0][i],k,P0gg,P1gg);

						//Nonsinglet
						for (j=32;j<=30+nf;j++)
							S[1][j][i+1][k]=RecRel_B(S[0][j][i],S[1][j][i],k,P0NS,P1NSp);

						if (order==1)
							for (j=13;j<=12+nf;j++)
								S[1][j][i+1][k]=RecRel_B(S[0][j][i],S[1][j][i],k,P0NS,P1NSm);
						else
							for (j=25;j<=24+nf;j++)
								S[1][j][i+1][k]=RecRel_B(S[0][j][i],S[1][j][i],k,P0NS,P1NSm);
					}
				...
				for (n=2;n<=trunc;n++)
				{
					...
					if (order==1)
					{
						//Evolution of NLO n-coefficients
						for (k=0;k<=GRID_PTS-2;k++)
						{
							//Singlet
							S[n][31][i+1][k]+=RecRel_B(S[n-1][31][i],S[n][31][i],k,P0qq,P1qq)+RecRel_B(S[n-1][0][i],S[n][0][i],k,P0qg,P1qg);

							S[n][0][i+1][k]+=RecRel_B(S[n-1][31][i],S[n][31][i],k,P0gq,P1gq)+RecRel_B(S[n-1][0][i],S[n][0][i],k,P0gg,P1gg);

							//Nonsinglet
							for (j=13;j<=12+nf;j++)
								S[n][j][i+1][k]+=RecRel_B(S[n-1][j][i],S[n][j][i],k,P0NS,P1NSm);

							for (j=32;j<=30+nf;j++)
								S[n][j][i+1][k]+=RecRel_B(S[n-1][j][i],S[n][j][i],k,P0NS,P1NSp);
						}
					}

					if (order==2)
					{
						//Evolution of NNLO n-coefficients
						for (k=0;k<=GRID_PTS-2;k++)
						{
							//Singlet
							S[n][31][i+1][k]+=RecRel_C(S[n-2][31][i],S[n-1][31][i],S[n][31][i],k,P0qq,P1qq,P2qq)+
												RecRel_C(S[n-2][0][i],S[n-1][0][i],S[n][0][i],k,P0qg,P1qg,P2qg);

							S[n][0][i+1][k]+=RecRel_C(S[n-2][31][i],S[n-1][31][i],S[n][31][i],k,P0gq,P1gq,P2gq)+
												RecRel_C(S[n-2][0][i],S[n-1][0][i],S[n][0][i],k,P0gg,P1gg,P2gg);

							//Nonsinglet
							S[n][25][i+1][k]+=RecRel_C(S[n-2][25][i],S[n-1][25][i],S[n][25][i],k,P0NS,P1NSm,P2NSv);

							for (j=26;j<=24+nf;j++)
								S[n][j][i+1][k]+=RecRel_C(S[n-2][j][i],S[n-1][j][i],S[n][j][i],k,P0NS,P1NSm,P2NSm);

							for (j=32;j<=30+nf;j++)
								S[n][j][i+1][k]+=RecRel_C(S[n-2][j][i],S[n-1][j][i],S[n][j][i],k,P0NS,P1NSp,P2NSp);
							}
						}
					}
			}
				...
				for (j=0;j<=30+nf;j++)
				{
					for (k=0;k<=GRID_PTS-2;k++)
					{
						...
						for (i=1;i<=ITERATIONS-1;i++)
							for (n=0;n<=trunc;n++)
								R[j][k]+=S[n][j][i][k]*pow(alpha1,(double)n)*pow(L1,(double)i)/fact(i);
					}

					if (j==0)
						j=( order<2? 12 : 24 );
					else if ( (j==12+nf) || (j==24+nf) )
						j=30;
				}
			...
			for (k=0;k<=GRID_PTS-2;k++)
			{
				if (order==2)
				{
					S[0][13][0][k]=S[0][25][0][k];
					for (j=26;j<=24+nf;j++) S[0][13][0][k]+=S[0][j][0][k];
					S[0][13][0][k]/=Nf;

					for (j=14;j<=12+nf;j++) S[0][j][0][k]=S[0][13][0][k]-S[0][j+12][0][k];
				}

				S[0][19][0][k]=S[0][31][0][k];
				for (j=32;j<=30+nf;j++) S[0][19][0][k]+=S[0][j][0][k];
				S[0][19][0][k]/=Nf;

				for (j=20;j<=18+nf;j++) S[0][j][0][k]=S[0][19][0][k]-S[0][j+12][0][k];

				for (j=1;j<=nf;j++)
				{
					S[0][j][0][k]=0.5*(S[0][j+18][0][k]+S[0][j+12][0][k]);
					S[0][j+6][0][k]=0.5*(S[0][j+18][0][k]-S[0][j+12][0][k]);
				}
			}
		}
		...
		if (HFT && vfns && order==2 && Q[nf+2]!=0.)
		{
			...
			for (k=0;k<=GRID_PTS-2;k++)
			{
				for (i=1;i<=nf;i++)
					for (j=i;j<=i+6;j+=6)
						S[0][j][0][k]+=pow(alpha1/(4.*Pi),2.)*convolution(k,A2ns,S[0][j][1]);

				S[0][0][0][k]+=pow(alpha1/(4.*Pi),2.)*(convolution(k,A2gq,S[0][31][1])+convolution(k,A2gg,S[0][0][1]));

				S[0][nf+1][0][k]=S[0][nf+7][0][k]=0.5*pow(alpha1/(4.*Pi),2.)*(convolution(k,A2hq,S[0][31][1])+convolution(k,A2hg,S[0][0][1]));
			}
		}
	}
}
